...
export default function MicroblogsPage() {
  ...
  const createMut = useMutation({
    ...
    onMutate: async (data) => {
      ...
      qc.setQueryData<Item[]>(["microblogs"], (old) => [
        ...(old || []),
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<Item> }) =>
      updateMicroblog(id, data as any),
        ...
        >
          <form
            ...
            onSubmit={async (e) => {
              ...
              if (editId) updateMut.mutate({ id: editId, data: { ...form, imageUrl } as any });
              else createMut.mutate({ ...form, imageUrl } as any);
            }}
            className="space-y-4"
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => s("published", !(form.published ?? false))}
            ...
            <div className="space-y-1.5">
              ...
              <ContentEditor
                key={drawerOpen ? (editId ?? "new") : "closed"}
                content={f("content")}
                onChange={(html) => s("content", html)}
                generateContext={{ title: f("title"), type: "microblog" }}
              />

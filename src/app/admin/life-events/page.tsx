...
export default function LifeEventsPage() {
  ...
  const createMut = useMutation({
    ...
    onMutate: async (data) => {
      ...
      qc.setQueryData<Item[]>(["life-events"], (old) => [
        ...(old || []),
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<Item> }) =>
      updateLifeEvent(id, data as any),
  ...
  const _selectCls = `${inputCls} appearance-none h-[34px]`;
  ...
  return (
    <div className="space-y-6">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {item.location && (
                          <p className="text-[11px] text-fg/40 mt-0.5">
                            <PushPin
                              weight="fill"
                              className="w-3 h-3 inline-block mr-0.5 align-text-top"
                            />
                            {item.location}
                          </p>
                        )}
      ...
      {items.length === 0 && (
        <p className="text-xs text-fg/50 text-center py-8">No life events yet.</p>
      )}
        ...
        >
          <form
            ...
            onSubmit={async (e) => {
              ...
              if (editId) updateMut.mutate({ id: editId, data: data as any });
          ...
          >
              ...
              <div className={`space-y-1.5 ${form.current ? "sm:col-span-2" : ""}`}>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`${inputCls} pr-8 dark:[color-scheme:dark]`}
                    required
                  />
                  {f("startDate") && (
                    <button
                      type="button"
                      onClick={() => s("startDate", "")}
                      className="absolute right-1 top-1/2 -translate-y-1/2 w-5 h-5 flex items-center justify-center text-fg/40 hover:text-fg transition-colors cursor-pointer"
                    >
            ...
            <div className="space-y-1.5">
              ...
              <ContentEditor
                ...
                generateContext={{ title: f("title"), type: "lifeEvent" }}
              />
